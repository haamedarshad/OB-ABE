...
void wellcome(){
	int algID = 0;
	printf("Please select one of the following options (just number):\n1. Setup\n2. Key Generation\n3. Encryption\n4. Decryption\n");
	while(algID == 0){
		scanf("%d", &algID);
		switch(algID){
			case 1:{
			...
			default:{
				algID = 0;
				printf("You have entered a wrong number!\nPlease select one of the following options (just number):\n1. Setup\n2. Key Generation\n3. Encryption\n4. Decryption\n");
			}
		}
	}
}
...
int initialize_enclave(void)
{
    ...
    ret = sgx_create_enclave(ENCLAVEOBABE_FILENAME, SGX_DEBUG_FLAG, &token, &updated, &global_eid, NULL);
...
/* OCall functions */
void ocall_EnclaveOBABE_sample(const char *str)
{
...
int SGX_CDECL main(int argc, char *argv[])
{
    ...
    ret = ecall_EnclaveOBABE_sample(global_eid, &ecall_return);

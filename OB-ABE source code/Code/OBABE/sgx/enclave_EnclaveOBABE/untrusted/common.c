...
/////////////////////////////
AES_KEY
init_aes_2( element_t k, int enc, AES_KEY* key, unsigned char* iv )
{
  AES_KEY key2;
  int key_len;
  unsigned char* key_buf;
  ...
  AES_set_decrypt_key(key_buf + 1, 128, &key2);
...
GByteArray* aes_128_cbc_encrypt_3( GByteArray* pt, element_t k, unsigned int* keyArray)
{
  ...
  key2= init_aes_2(k, 1, &key, iv);

  memcpy(keyArray, key2.rd_key, 244);
...
memcpy(key.rd_key, keyArray, 244);
...
/////////////////////For OB-ABE
void read_cpabe_file_OBABE( char* file,    GByteArray** cph_buf, int* file_len, GByteArray** aes_buf, GByteArray** kSgxAbe)
{
FILE* f;
int i;
int len;
...
for( i = 3; i >= 0; i-- )
len |= fgetc(f)<<(i*8);
g_byte_array_set_size(*kSgxAbe, len);
fread((*kSgxAbe)->data, 1, len, f);
...
///////////////For OB-ABE
void
write_cpabe_file_OBABE(char* file,GByteArray* cph_buf,int file_len, GByteArray* aes_buf, GByteArray* kSgxAbe)
{
FILE* f;
int i;
f = fopen_write_or_die(file);
...
/// write kSgxAbe / The encryption key which is secured using SGX and ABE

for( i = 3; i >= 0; i-- )
fputc((kSgxAbe->len & 0xff<<(i*8))>>(i*8), f);

fwrite(kSgxAbe->data, 1, kSgxAbe->len, f);

...
//////////////////////////////////
	//If there is no any obligations, then this part makes a problem. So, an IF condition has been added to fix the problem
	int b=0;
	fread(&b, 1, 1, f);
	if(b == 0)
		fseek(f,-1,SEEK_CUR);
									...
									int file_len, GByteArray* aes_buf)
{
	...
	/////If there is any obligation, then we need to write the obligation in the file
	int aa=0;
	aa=aes_buf->obg;
	if(aa==17||aa==18||aa==19)
		fwrite(&aa, 1, 1, f);
	////////
	/* write cph_buf */
	for( i = 3; i >= 0; i-- )
